...
flags.DEFINE_integer("steps_to_validate", 300,
...
flags.DEFINE_string("inference_test_file", "../data/a8a/a8a_test.libsvm.tfrecords",
...
def generate_batches(FLAGS, filename_queue, is_train=True):
    batch_threads = FLAGS.batch_thread_number
    capacity = batch_threads * FLAGS.batch_size + FLAGS.min_after_dequeue
...
def lr_inference(FLAGS, sparse_ids, sparse_values, is_train=True):
    input_units = FLAGS.feature_size
    output_units = FLAGS.label_size
...
def setup_optimizer(FLAGS, global_step):
    # set up optimizer
...
def setup_loss(FLAGS, logits, batch_labels):
    if FLAGS.label_size > 1:
        ...
        logits2 = tf.squeeze(logits, axis=1)
...
def create_dir(path):
    if not path.startswith("fds://") and not os.path.exists(
            path):
        os.makedirs(path)


def create_dirs(FLAGS):
    create_dir(FLAGS.checkpoint_path)
    create_dir(FLAGS.saved_model_path)
    create_dir(FLAGS.output_path)


def restore_session_from_checkpoint(sess, saver, checkpoint_path):
...
def libsvm_generator(file_name):
    for line in open(file_name, "r"):
        ...
        yield [label], feature_ids, feature_values


def tfrecord_generator(file_name):
...
def evaluation(FLAGS, sess):
    ...
    if not restore_session_from_checkpoint(sess, saver, FLAGS.checkpoint_path):
        logging.error("No checkpoint found, exit now")
        exit(1)

    # Load inference test data
    resulte_file_name = FLAGS.inference_result_file
    test_file_name = FLAGS.inference_test_file

    batch_sz = FLAGS.validate_batch_size
    batches = 0
    ...
    with open(resulte_file_name, 'w') as out:
        batch_feature_index = []
        batch_labels = []
        batch_ids = []
        batch_values = []
        ...
        for label, ids, values in tfrecord_generator(test_file_name):
...
def train_validate(FLAGS, sess):
    ...
    restore_session_from_checkpoint(sess, saver, FLAGS.checkpoint_path)
